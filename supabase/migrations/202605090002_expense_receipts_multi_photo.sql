...
set receipts = jsonb_build_array(
  jsonb_strip_nulls(
    jsonb_build_object(
      ...
      'type', receipt_content_type
    )
  )
)
where coalesce(jsonb_array_length(receipts), 0) = 0
...
create or replace function public.update_expense_with_equal_split(
  ...
  p_participant_member_ids uuid[],
  p_note text default null,
  p_clear_receipts boolean default false,
...
begin
  ...
  set
    ...
    receipts = case
      when p_clear_receipts then '[]'::jsonb
      when p_receipts is not null then p_receipts
      else receipts
    end,
...
grant execute on function public.create_expense_with_equal_split(uuid, text, numeric, text, uuid, text, uuid[], text, jsonb) to authenticated;
grant execute on function public.update_expense_with_equal_split(uuid, text, numeric, uuid, text, uuid[], text, boolean, jsonb) to authenticated;
